...
function setUserIdentity() {
  ...
  app.color = elements.colorInput.value || localStorage.getItem("realtime-color") || randomColor();
  app.userId = localStorage.getItem("realtime-user-id") || crypto.randomUUID();

  localStorage.setItem("realtime-username", app.username);
  localStorage.setItem("realtime-color", app.color);
  localStorage.setItem("realtime-user-id", app.userId);
...
function bootstrap() {
  ...
  elements.colorInput.value = savedColor || "#7c3aed";
  updateTheme();
  wireEvents();
  setUserIdentity();
}
